...
const ToastIcon = ({ config, }) => (_jsx("span", { className: "shrink-0", style: { color: config.iconColor }, children: _jsx(AppIcon, { iconName: config.iconName, source: config.iconSource, size: 18, strokeWidth: 1.5 }) }));
const CloseButton = ({ toastId }) => (_jsx(Button, { variant: "ghost", onClick: () => toast.dismiss(toastId), className: "bg-transparent border-none text-tertiary-text cursor-pointer p-1 shrink-0 rounded-md", children: _jsx(AppIcon, { iconName: "x", size: 18, strokeWidth: 1.5 }) }));
...
export const ActionableToastContent = ({ id, title, description, variant = "success", duration = 5000, showProgress = true, colors, className, action, actions, }) => {
    ...
    return (_jsx("div", { className: cn("rounded-lg shadow-lg p-px w-[380px] max-w-[380px] overflow-hidden", className), style: { background: config.borderGradient }, children: _jsxs("div", { className: "rounded-lg overflow-hidden", style: { background: config.bgColor }, children: [_jsxs("div", { className: "px-2 py-2 pb-1 flex flex-col gap-2", children: [_jsxs("div", { className: "flex items-start gap-2", children: [_jsx(ToastIcon, { config: config }), _jsxs("div", { className: "flex-1 min-w-0 flex flex-col gap-1", children: [_jsx("span", { className: "font-medium text-primary-text text-xs leading-tight", children: title }), description && (_jsx("span", { className: "text-tertiary-text text-xs font-normal", children: description }))] }), !isProcessing && _jsx(CloseButton, { toastId: id })] }), _jsx("div", { className: "flex justify-end gap-2 ", children: resolvedActions.map((act, idx) => {
