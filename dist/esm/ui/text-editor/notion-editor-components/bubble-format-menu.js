...
export const BubbleFormatMenu = ({ editor }) => {
    ...
    return (_jsxs("div", { className: "relative", children: [_jsxs("div", { className: "flex items-center gap-1 bg-agent-card-fill rounded-lg p-1.5 shadow-2xl border border-border-3", children: [NOTION_BUBBLE_FORMAT_MENU_TOOLBAR_CONFIG.map((format, idx) => {
                        ...
                        return (_jsx(Button, { onClick: () => format.action(editor), title: format.label, className: `p-2 rounded transition-colors text-tertiary-text cursor-pointer bg-transparent ${isActive
                        ...
                        }, title: "Font Size", className: `p-2 rounded transition-colors flex items-center gap-1 text-tertiary-text cursor-pointer bg-transparent ${showFontSizePicker
                            ? "bg-primary-foreground/10 text-primary-text"
