...
export const TextEditorFontSizePicker = ({ editor, onClose }) => {
    ...
    return (_jsxs("div", { className: "absolute top-full w-full mt-2 bg-agent-card-fill border border-border-3 rounded-lg shadow-xl p-2 z-50 min-w-[160px]", children: [_jsx("div", { className: "text-sm font-medium text-primary-text mb-2 px-2", children: "Font Size" }), _jsxs("div", { className: "space-y-1", children: [fontSizes.map((size) => (_jsxs(Button, { onClick: () => {
                            ...
                            onClose();
                        }, className: "w-full px-3 py-2 text-left rounded transition-colors text-xs text-tertiary-text hover:text-primary-text hover:bg-primary-foreground/10 font-light flex items-center justify-between cursor-pointer duration-100 bg-transparent", children: [_jsx("span", { className: "font-medium", children: size.label }), _jsxs("span", { children: ["(", size.value, ")"] })] }, size.value))), _jsx("div", { className: "border-t border-button-border2 my-1" }), _jsx(Button, { onClick: () => {
