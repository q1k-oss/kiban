...
const BorderMovingWrapper = ({
  ...
  useEffect(() => {
    ...
    const newLines = generateLines(
      size.width, size.height, borderRadius, effectiveSegments,
      colors, startOffset, strokeWidth, borderPosition, parsedBorderOffset
    );
    setLines(newLines);
  ...
  useEffect(() => {
    if (isActive && animate && !animationStarted.current) {
      ...
      setAnimationCompleted(false);
      if (!showWhenInactive) setVisibleCount(0);
    }
  ...
  const displayLines = useMemo(() => {
    ...
    if (variant === 'split') {
      ...
      return lines.map((line, i) => ({
        ...line,
        color: getColorAtPosition(colors, i < halfSegments
          ? i / halfSegments
  ...
  useEffect(() => {
    ...
    for (const line of lineElements) {
      ...
      cumulative += Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
    }
    ...
    const tick = (now: number) => {
      ...
      for (let i = 0; i < numLines; i++) {
        ...
        if (!drawInVisible) {
          if (prevVisible[i] !== false) { lineElements[i].style.opacity = '0'; prevVisible[i] = false; }
          continue;
        }
        if (isTip) {
          if (prevVisible[i] !== true) { lineElements[i].style.opacity = '1'; prevVisible[i] = true; }
          continue;
        }
    ...
    >
      ...
      {size.width > 0 && displayLines.length > 0 && shouldShowBorder && (
        <BorderSvg
          ...
          animate={animate}
          ants={ants}
          parsedBorderOffset={parsedBorderOffset}
          borderPosition={borderPosition}
          antsGroupRef={antsGroupRef}
        />
      )}
    </div>
  );
