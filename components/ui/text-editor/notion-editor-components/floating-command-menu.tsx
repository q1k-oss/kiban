...
export const FloatingCommandMenu = ({ editor }: FloatingMenuProps) => {
  return (
    <div
      className="bg-agent-card-fill border border-border-3 rounded-lg shadow-2xl w-80 max-h-96 overflow-y-auto"
      role="menu"
      aria-label="Text formatting commands"
    >
      <div className="p-1 flex flex-col items-center w-full gap-1">
        {NOTION_FLOAT_MENU_TOOLBAR_CONFIG.map((cmd, idx) => (
          ...
          >
            <span className="mt-0.5 text-icon-disabled-fill group-hover:text-primary-text duration-100">
